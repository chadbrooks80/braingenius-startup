...
The current task being worked on is stored in `@context/current-feature`.
...
1. Read `@context/current-feature`
...
- Always begin by reading `@context/current-feature`
...
- Use the contents of `@context/current-feature` to determine:
  - the branch name
  - the scope of work
...
- Base the branch name on the feature described in `@context/current-feature`
...
- Build only the requested feature or fix from `@context/current-feature`
