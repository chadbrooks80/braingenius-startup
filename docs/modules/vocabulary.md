...
- Panel setter registration: `tests/learning-engine/vocabularyStatusPanel.test.tsx` proves `createVocabularyStatusPanelSetters()` bundles exactly the three real setters by identity, the panel's initial zero-count/empty-message render, and each section's populated-vs-empty rendering (including that Spelling always renders masked rows, never word text). `tests/learning-engine/modulePanelRegistration.test.ts` proves the generic `LearningEngine.registerModulePanelSetters()` bridge stores, replaces, and safely disposes an opaque registration by token (an old/Strict-Mode-style disposer can never clear a newer registration), independent of Vocabulary. This repository's static-render component-test harness cannot mount/unmount a component or fire its effects, so the registration effect actually firing on mount/unmount is not provable here and is a documented limitation.
