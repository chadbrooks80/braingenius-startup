...
Both the browser lesson state and server capability/attempt state are memory-only. Page refresh creates a new lesson. Server restart loses capabilities, attempts, idempotency records, and progress. No Prisma model persists learning history. The singleton store is not suitable for multiple server instances.

## Tests

- State/progression: `tests/vocabulary/VocabularyLessonState.test.ts`, `VocabularyActiveAttempt.test.ts`, `Vocabulary.test.ts`.
...
- Window/submission: Learning Engine flow, Multiple Choice, and Spelling test folders.
- Full flow: `tests/integration/vocabularyRouteSmoke.test.ts` and `tests/e2e/vocabularyRoute.e2e.ts`.
