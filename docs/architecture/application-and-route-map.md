...
| `/getting-started` | `src/app/(auth)/(onboarding)/getting-started/page.tsx` | Server | Requires a server session and existing user, computes the authoritative onboarding route, and renders one onboarding step in `OnboardingShell`. Proxy also redirects signed-in users whose funnel target differs. |
