...
node --import ./tests/registerServerOnly.mjs --import tsx --test tests/billing/entitlement.test.ts tests/billing/userEntitlement.test.ts
...
- `tests/billing/`: strict entitlement time/status/price boundaries; Checkout ownership, price, mode, quantity, payment, subscription, persistence, idempotency, and transient-failure confirmation; real webhook signature/dispatch/failure behavior plus fail-closed subscription synchronization; and (`userEntitlement.test.ts`) the paid-TTS entitlement resolver's direct/child-inherited grants, stable multi-parent principal selection, manual-suspension denial, and database-failure fail-closed behavior.
